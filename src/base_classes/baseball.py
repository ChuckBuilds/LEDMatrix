...
class BaseballRecent(Baseball):
    """Base class for recent baseball games."""
    
    def __init__(self, config: Dict[str, Any], display_manager, cache_manager, logger: logging.Logger, sport_key: str):
        super().__init__(config, display_manager, cache_manager, logger, sport_key)
        self.logger.info(f"{sport_key.upper()} Recent Manager initialized")
    
    def _should_show_baseball_game(self, game: Dict) -> bool:
        """Determine if a recent baseball game should be shown."""
        try:
            # Only show final games
...
class BaseballUpcoming(Baseball):
    """Base class for upcoming baseball games."""
    
    def __init__(self, config: Dict[str, Any], display_manager, cache_manager, logger: logging.Logger, sport_key: str):
        super().__init__(config, display_manager, cache_manager, logger, sport_key)
        self.logger.info(f"{sport_key.upper()} Upcoming Manager initialized")
    
    def _should_show_baseball_game(self, game: Dict) -> bool:
        """Determine if an upcoming baseball game should be shown."""
        try:
            # Only show upcoming games
            if not game.get('is_upcoming', False):
                return False
